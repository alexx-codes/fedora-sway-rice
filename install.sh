...
deploy_configs() {
    ...
    # Icon theme comes from the one palette definition.
    if [ -f "$repo/theme/colors.env" ] && [ -f "$cfg/qt6ct/qt6ct.conf" ]; then
        local icons
        icons=$(sed -n 's/^ICON_THEME=//p' "$repo/theme/colors.env")
        [ -n "$icons" ] && sed -i "s/^icon_theme=.*/icon_theme=$icons/" "$cfg/qt6ct/qt6ct.conf"
    fi
    ...
    cp "$cfg/rice/theme/swaylock.conf" "$cfg/swaylock/config"

    # waybar/swaync import a relative theme.css; make it a symlink into active/
    cp "$cfg/rice/theme/colors.css" "$cfg/waybar/theme.css"
    cp "$cfg/rice/theme/colors.css" "$cfg/swaync/theme.css"
    ...
    cp "$cfg/rice/theme/rofi.rasi" "$cfg/rofi/config.rasi"
...
EOF
    ...
    # Wallpaper links, which swaylock and the wallpaper daemon both read.
    if [ ! -e "$cfg/rice/wallpapers/current" ]; then
        local wallname wallfile ext
        wallname=$(sed -n 's/^WALLPAPER=//p' "$cfg/rice/theme/colors.env" 2>/dev/null)
        wallfile=""
        for ext in jpg jpeg png; do
...
post_setup() {
    ...
    . "$HOME/.config/rice/theme/colors.env"
